...
> **Multi-destination:** A single Source Plant can serve multiple Target Areas in the same week. Each Source → Target Area combination is a separate `APLSynergyPlanHeader` record (uniquely identified by `Year, Week, SourcePlantCode, TargetAreaCode`). To plan shipments to a second Target Area, the planner returns to this index page and creates a new plan with the same Source Plant but a different Target Area selection. The Projected WH Stock on each detail page already deducts allocations committed in **all other** saved Synergy plans for the same Source + Week + Year, regardless of which transport mode was used.
...
$$\text{Projected}[fa][d] = \text{whsOHSBox}[fa] + \sum_{i=0}^{d} \text{wppBox}[fa][i] - \sum_{i=0}^{d} \text{totalTransfer}[fa][i] - \sum_{i=0}^{d} \text{otherAreaTransfer}[fa][i]$$
...
- `totalTransfer[fa][d]` = `Σ transfer[fa][T][d]` for all trips T1..T{NVeh} — total boxes dispatched on day d in **this** plan (current Target Area).
- `otherAreaTransfer[fa][d]` = total boxes already allocated to **other Target Areas** from the same Source Plant for this FA on day d. Loaded at Calculate time from all other saved Synergy plans sharing the same `(SourcePlantCode, Week, Year)`. Read-only context — to adjust, open the corresponding plan from the index page.

Day index d = 0 = Monday. Positive = WH has remaining stock. Negative = WH overcommitted.

**Other-area transfer query** — fetched per FA per day during Calculate:

```sql
SELECT
    a.FaCode,
    d.DayOfWeek,
    SUM(a.QtyBox) AS OtherAreaBox
FROM APLSynergyPlanHeader h
JOIN APLSynergyPlanDay        d ON d.PlanHeaderId = h.Id
JOIN APLSynergyPlanTrip       t ON t.PlanDayId    = d.Id
JOIN APLSynergyPlanAllocation a ON a.PlanTripId   = t.Id
WHERE h.SourcePlantCode = @SourcePlantCode
  AND h.Week            = @Week
  AND h.Year            = @Year
  AND h.TargetAreaCode  <> @TargetAreaCode
GROUP BY a.FaCode, d.DayOfWeek
```

Result mapped to `otherTransfer[fa][d]` (zero-indexed, Mon=0). Missing rows default to 0. This query is transport-agnostic — it sums across all modes and vehicles used by other plans for the same source.

**Transfer input validation:** When `totalTransfer[fa][d]` (in this plan) combined with `otherAreaTransfer[fa][d]` would make `Projected[fa][d] < 0`, those transfer input cells receive the `input-over-proj` CSS class (red border + light-red background) and the Projected cell renders in red. The planner corrects manually — no auto-clamping is applied. Save remains enabled to allow saving partial work.
...
3. Fetches other-area transfers: queries `APLSynergyPlanAllocation` joined through `APLSynergyPlanTrip → APLSynergyPlanDay → APLSynergyPlanHeader` for all plans with the same `SourcePlantCode + Week + Year` but a different `TargetAreaCode`. Result stored as `otherTransfer[fa][d]` on each FA object.
4. For weeks with pre-existing plan records: reloads saved transfer quantities, NVeh, and transport selections; refreshes only WH OHS and WPP data.
5. For weeks with no plan record yet: creates a new plan record with all transfer quantities defaulted to 0.
6. Computes Projected WH Stock (including `otherAreaTransfer` deduction), WH Surge, and per-trip LF for all 4 weeks simultaneously.
...
| Other-area plans already consume enough WH stock to make `Projected[fa][d] < 0` even with zero transfer in this plan | Projected cell renders red; `input-over-proj` applied. Planner must return to index page and reduce allocations in the other-destination plan(s) for the same Source + Week + Year. |
